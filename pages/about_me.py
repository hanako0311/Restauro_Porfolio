...
def show_about_me():
    ...
    with body:
        ...
        st.markdown(
            "<h1 style='font-size: 4rem; font-weight: bold; color: #dc143c;'>HI, I'm Hannah Restauro</h1>",
        ...
        with col1:
            ...
            st.write(info["brief"])
            st.markdown(
                f"###### 🧑‍🎓 Who I Am: {info['name']} - A {info['study']} student"
            )
            st.markdown(f"###### 🏙️ Where I'm Based: {info['location']}")
            st.markdown(f"###### 🚀 What Drives Me: {info['interest']}")
            st.markdown(
                "###### 🔴 My Favorite Color: Crimson Red - symbolizing energy and determination"
            )
            st.markdown(
                f"###### 🔗 Let's Connect: [LinkedIn]({info['https://www.linkedin.com/in/hnnhrstr/']})"
            )
